...
const osg::Quat MISSILE_BASIS(0.0f, 0.0f, 1.0f, 1.2168e-08f); // AIM-9L initial rotation
...
static osg::Quat frameAlignQuat(const osg::Vec3 &forward_world, const osg::Vec3 &up_world)
{
    ...
    const osg::Vec3 Xl(1, 0, 0); // local forward
    const osg::Vec3 Zl(0, 0, 1); // local up
    osg::Vec3 Yl = Zl ^ Xl;
    Yl.normalize();
...
public:
    ...
    void operator()(osg::Node *node, osg::NodeVisitor *nv) override
    {
        ...
        if (isMissile)
        {
            // Apply missile’s base rotation
            finalRot = MISSILE_BASIS * orient;
        }
        else
        {
            // Apply aircraft’s base orientation and roll flip
            finalRot = ROLL_180 * F14_BASIS * orient;
        }

        osg::Matrix M = osg::Matrix::rotate(finalRot) * osg::Matrix::translate(pos);
        mt->setMatrix(M);

        // Collision check (missile vs aircraft)
        if (!isMissile)
        {
