...
int main(int argc, char **argv)
{
    ...
    // Replace the existing tailNode->setMatrix() with these offsets:
    tailNode->setMatrix(
        osg::Matrix::rotate(
            osg::Quat(-0.0984102, 0.0984102, -0.700225, 0.700225)) // rotation
        * osg::Matrix::translate(-8.0f, 1.0f, 3.0f)                // translation
    );

    // wingNode->setMatrix(osg::Matrix::translate(-10.0f, 0.0f, 2.0f));
    wingNode->setMatrix(
        osg::Matrix::rotate(
            osg::Quat(0.0, 0.022433, -0.642396, 0.766044)) // rotation
        * osg::Matrix::translate(65.5f, 2.5f, 2.0f)        // translation
    );

    // cockpitNode->setMatrix(osg::Matrix::translate(0.0f, 5.0f, 2.0f));
    cockpitNode->setMatrix(
        osg::Matrix::rotate(
            osg::Quat(-0.0500815, 0.0523467, -0.715505, 0.694841)) // rotation
        * osg::Matrix::translate(63.5f, -2.0f, 2.0f)               // translation
    );
