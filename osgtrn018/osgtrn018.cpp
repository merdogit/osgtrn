...
public:
    CameraSwitchHandler(osgGA::NodeTrackerManipulator *manip,
        ...
        : _manip(manip), _tail(tail), _wing(wing), _cockpit(cockpit), _top(top),
...
osg::AnimationPath *createAnimationPath(float radius, float time)
{
    ...
    for (unsigned int i = 0; i < numSamples; ++i)
    {
        ...
        // Add small roll oscillation for realism
        float roll = sinf(delta_time * i * 4.0f) * osg::DegreesToRadians(5.0f);
        osg::Quat rollQuat(roll, osg::Vec3(0, 1, 0));

        path->insert(delta_time * (float)i, osg::AnimationPath::ControlPoint(pos, rot * rollQuat));
    }

    return path.release();
}

// ===== Reference circle for animation =====
...
int main(int argc, char **argv)
{
    ...
    // Reference circle
    root->addChild(createReferenceCircle(radius));
    ...
    // Top marker: 50 units above Cessna
    // topNode->setMatrix(osg::Matrix::translate(0.0f,0.0f,50.0f));
    topNode->setMatrix(osg::Matrix::rotate(osg::Quat(0.489579, 0.362846, -0.163801, 0.775773)) * 
    osg::Matrix::translate(0, -47.5, 50));
